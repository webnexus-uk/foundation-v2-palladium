...
describe("Test pool functionality", () => {
      ...
      if (type !== "debug") {
        ...
        if (response.length === 2) {
          ...
          expect(response[0][1]).toBe(
            "Submitted a primary block (Palladium:1) successfully to Palladium's daemon instance(s)"
          );
    ...
    pool.on("pool.log", (type, text) => {
      if (type !== "debug") {
        ...
        if (response.length === 2) {
          ...
          expect(response[1][1]).toBe(
            "Requested template from primary chain (Palladium:2) via RPC polling"
          );
          done();
        }
      }
      ...
      if (type !== "debug") {
        ...
        if (response.length === 3) {
          ...
          expect(response[2][1]).toBe(
            "Requested template from primary chain (Palladium:1) via RPC polling"
          );
          done();
        }
      }
    ...
    pool.on("pool.log", (type, text) => {
      if (type !== "debug") {
        ...
        if (response.length === 2) {
          ...
          expect(response[1][0]).toBe("log");
          expect(response[1][1]).toBe(
            "Difficulty update queued for worker: 1BvBMSEYstWetqTFn5Au4m4GFg7xJaNVN2 (8)"
          );
          pool.network.stopNetwork();
        }
      }
    ...
    pool.on("pool.log", (type, text) => {
      if (type !== "debug") {
        ...
        if (response.length === 2) {
          ...
          expect(response[1][0]).toBe("log");
          expect(response[1][1]).toBe(
            "Difficulty updated successfully for worker: 1BvBMSEYstWetqTFn5Au4m4GFg7xJaNVN2 (8)"
          );
          pool.network.stopNetwork();
        }
      }
  ...
  test("Test pool stratum authentication [2]", (done) => {
    ...
    mockSetupDaemons(pool, () => {
      ...
      pool.checkWorker(
        pool.primary.daemon,
        "bc1qar0srrr7xfkvy5l643lydnw9re59gtzzwf5mdq.worker1",
        (valid) => {
          expect(valid).toBe(true);
          done();
        }
  ...
  test("Test pool workers handling [1]", () => {
    ...
    const expected = {
      "1BvBMSEYstWetqTFn5Au4m4GFg7xJaNVN2": { immature: 10000, generate: 0 },
    };
  ...
  test("Test pool workers handling [2]", () => {
    ...
    const expected = {
      "1BvBMSEYstWetqTFn5Au4m4GFg7xJaNVN2": { generate: 0, immature: 20000 },
    };
  ...
  test("Test pool workers handling [5]", () => {
    ...
    const expected = {
      "1BvBMSEYstWetqTFn5Au4m4GFg7xJaNVN2": { generate: 10000, immature: 0 },
    };
    pool.handlePrimaryWorkers([blocks1], [[worker1]], false, (results) => {
      expect(results).toStrictEqual(expected);
    });
  ...
  test("Test pool workers handling [6]", () => {
    ...
    const expected = {
      "1BvBMSEYstWetqTFn5Au4m4GFg7xJaNVN2": { immature: 0, generate: 20000 },
    };
    pool.handlePrimaryWorkers(
      [blocks1, blocks1],
      [[worker1], [worker1]],
  ...
  test("Test pool workers handling [9]", () => {
    ...
    const expected = {
      "1BvBMSEYstWetqTFn5Au4m4GFg7xJaNVN2": { immature: 0, generate: -10000 },
    };
    pool.handlePrimaryWorkers([blocks1], [[worker1]], false, (results) => {
      expect(results).toStrictEqual(expected);
    });
